...
def time_split(df):
    ...
    """
    df['time_division'] = 0
    df.loc[(df['obs_time'] < 6), 'time_division'] = 1
    df.loc[(df['obs_time'] >= 6) & (df['obs_time'] < 20), 'time_division'] = 2
    df.loc[(df['obs_time'] >= 20) & (df['obs_time'] < 23), 'time_division'] = 3
...
def expending_sum(df, col):
    x = df.groupby(['Case'])[col].expanding().sum().reset_index().drop(['Case'], axis=1)
...
def expanding_timeslot_train_ver(train, train_x):
    ...
    train_x = train_x.sort_values(
        by=['Case', 'DAT', '시간대'], axis=0).reset_index()
    train_y_ready = train_y_ready.sort_values(
        by=['Case', 'DAT', '시간대'], axis=0).reset_index()
    ...
    for col in col_list:
        x = train_x.groupby(['Case'])[col].expanding(
...
def expanding_timeslot_test_ver(test_x):
    ...
    for col in col_list:
        x = test_x.groupby(['Case'])[col].expanding(
        ).sum().reset_index().drop(['Case'], axis=1)
        x.drop(['level_1'], axis=1, inplace=True)
        test_x[col] = x
...
def column_log(df):
    ...
    for col in col_list:
        if col in ['DAT', 'Case']:
            continue
        df[col+'_log'] = df[col].apply(lambda x: math.log(x+1))


def LPF(df, low, order=1) :
    new_df = pd.DataFrame()
    df_x_fill = df.iloc[:,1:33]
    case_list = df['Case'].unique()
    b, a = butter(
                    N = order,
                    Wn = low,
